...
//        MeshPtr meshFromGeometryGroup(const Group& g)const;
//        MeshPtr meshFromMaterialGroup(const Group& g)const;
  ...
  MeshPPtr meshPFromObj(const obj::ModelPtr& model);
  MeshPTPtr meshPTFromObj(const obj::ModelPtr& model);
  MeshPNPtr meshPNFromObj(const obj::ModelPtr& model);
  MeshPTNPtr meshPTNFromObj(const obj::ModelPtr& model);
  MeshPPtr indexedMeshPFromObj(const obj::ModelPtr& model);
  MeshPTPtr indexedMeshPTFromObj(const obj::ModelPtr& model);
  MeshPNPtr indexedMeshPNFromObj(const obj::ModelPtr& model);
  MeshPTNPtr indexedMeshPTNFromObj(const obj::ModelPtr& model);
...
#if 0
  template <typename V>
    shared_ptr<Mesh<V> > Mesh<V>::meshFromGeometryGroup(const Group& g)const
    {
      MeshPtr mesh(new Mesh<V>());
      assert(_indices.empty());
      mesh->_vertices.assign(_vertices.begin() + g.begin(),
          _vertices.begin() + g.end());

      std::vector<Group> groups = _materialGroups;
      std::vector<Group>::iterator last = partition(groups.begin(), groups.end(), 
          bind(&Group::contains, boost::cref(g), boost::cref(_1)));

      mesh->_materialGroups.reserve(last - groups.begin());
      transform(groups.begin(), last, back_inserter(mesh->_materialGroups),
